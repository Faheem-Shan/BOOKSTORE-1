...
// Updated to point to your Django Category endpoint
...
const AdminCategoryList = () => {
    ...
    const fetchCategories = async () => {
        ...
        try {
            ...
            toast.error('Error loading categories. Check if Django is running.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id, name) => {
        ...
        try {
            ...
            toast.error('Error deleting category. It might be linked to products.');
        }
    };
    ...
    const handleEditSave = async (category) => {
        ...
        try {
            ...
            setEditingId(null);
            toast.success(`Updated to "${updatedCategory.name}"`);
        } catch (error) {
            toast.error('Error updating category.');
        }
    };
    ...
    return (
        <div className="max-w-4xl mx-auto p-8 bg-white rounded-2xl shadow-xl border border-gray-50 my-10">
            <h2 className="text-3xl font-extrabold mb-8 text-gray-900 border-b pb-4">Book Categories</h2>

            {/* Add New Category Form */}
            <form onSubmit={handleAddCategory} className="flex flex-col sm:flex-row gap-3 mb-10 p-4 rounded-xl bg-gray-50 border border-gray-200">
                <input
                    type="text"
                    placeholder="e.g., Science Fiction"
                    value={newCategoryName}
                    onChange={(e) => setNewCategoryName(e.target.value)}
                    className="flex-1 px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-black outline-none transition-all"
                />
                <button
                    type="submit"
                    className="flex items-center justify-center px-6 py-3 bg-black text-white font-bold rounded-lg hover:bg-gray-800 transition-transform active:scale-95"
                >
            ...
            <div className="overflow-hidden border border-gray-100 rounded-xl shadow-sm">
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest w-1/4">ID</th>
                            <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest w-1/2">Category Name</th>
                            <th className="px-6 py-4 text-center text-xs font-bold text-gray-500 uppercase tracking-widest w-1/4">Actions</th>
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-100">
                        {categories.map((category) => (
                            <tr key={category.id} className="hover:bg-gray-50/50 transition-colors">
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-400">#{category.id}</td>
                                <td className="px-6 py-4">
                                    {editingId === category.id ? (
                                        <input
                                            type="text"
                                            value={editingName}
                                            onChange={(e) => setEditingName(e.target.value)}
                                            onKeyDown={(e) => e.key === 'Enter' && handleEditSave(category)}
                                            className="w-full px-3 py-1 border-2 border-black rounded-md outline-none"
                                            autoFocus
                                        />
                                    ) : (
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    {editingId === category.id ? (
                                        <button onClick={() => handleEditSave(category)} className="text-green-600 font-bold hover:underline mr-4">Save</button>
                                    ) : (
                                        <button onClick={() => startEdit(category)} className="text-indigo-600 hover:text-indigo-900 mr-4 transition-transform hover:scale-110">
                                            <FaEdit className="inline h-5 w-5" />
                                        </button>
                                    )}
                                    <button onClick={() => handleDelete(category.id, category.name)} className="text-red-500 hover:text-red-700 transition-transform hover:scale-110">
                                        <FaTrash className="inline h-4 w-4" />
                                    </button>
                                </td>
                            </tr>
