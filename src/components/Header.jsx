    ...
    <header className="sticky top-0 z-40 bg-white shadow-sm ">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-3">
        ...
        <Link to="/" className="flex items-center gap-2">
          <img src={logoImg} alt="Bookly" className="h-7" />
          <span className="text-xl font-semibold">
            Bookl<span className="text-blue-600">y.</span>
          </span>
        </Link>

        {/* NAV (DESKTOP) */}
        <nav className="hidden lg:flex gap-2 bg-gray-100 rounded-full px-2 py-1">
          {navItems.map(({ to, label }) => (
            <NavLink
              key={label}
              to={to}
        ...
        <div className="flex items-center gap-5">
          ...
          {/* WISHLIST */}
          <Link to={user ? "/wishlist" : "/login"} className="relative">
            <TbHeart className="text-2xl hover:text-red-500" />
          ...
          <Link to={user ? "/cart" : "/login"} className="relative">
            <TbShoppingBag className="text-2xl hover:text-blue-600" />
          ...
          {user ? (
            ...
            >
              ...
              <img
                src={userImg}
                alt="Profile"
                className="w-8 h-8 rounded-full cursor-pointer ring-2 ring-blue-500/50"
              />

              {/* DROPDOWN */}
              {showProfile && (
                <div className="absolute right-0 top-full pt-2">
                  <div className="w-44 bg-white border rounded-lg shadow-lg overflow-hidden">
            ...
            <Link
              to="/login"
              className="hidden sm:flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-full hover:bg-blue-700"
            >
              <RiUserLine />
              Login
            </Link>
      ...
      {menuOpened && (
        <div className="lg:hidden bg-white border-t p-4">
          ...
          {user ? (
            <button
              onClick={handleLogout}
              className="mt-2 w-full text-left px-3 py-2 text-red-600 hover:bg-red-50 rounded"
            >
            ...
            <Link
              to="/login"
              onClick={() => setMenuOpened(false)}
              className="mt-2 block px-3 py-2 text-blue-600 hover:bg-blue-50 rounded"
            >
              Login / Sign Up
            </Link>
          )}
